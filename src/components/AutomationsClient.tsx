  ...
  const runNow = () =>
    startTransition(async () => {
      ...
      if (res.error) setNotice({ ok: false, text: res.error });
        ...
        setNotice({
          ok: true,
          text: t(locale, 'track8.runNowDone')
          ...
          <Card key={rule.id} className="p-5 md:p-6">
            <div className="flex items-start justify-between gap-4">
              ...
              <button
                type="button"
                disabled={pending}
                onClick={() => toggle(rule, !rule.enabled)}
                aria-pressed={rule.enabled}
                className={`shrink-0 inline-flex items-center rounded-full px-3 py-1.5 text-xs font-bold transition-colors ${
                  rule.enabled
                    ? 'bg-emerald-100 text-emerald-800'
                    : 'bg-zinc-100 text-zinc-500 hover:bg-zinc-200'
                }`}
              >
                {rule.enabled ? t(locale, 'track8.enabled') : t(locale, 'track8.disabled')}
              </button>
            </div>
                ...
                {rule.trigger === 'QUOTE_AWAITING' && (
                  <label className="text-xs font-semibold text-zinc-700">
                    ...
                    <input
                      ...
                      className="ml-2 w-20 px-2.5 py-1.5 rounded-lg border border-zinc-200 bg-zinc-50 text-sm"
                    />
                  </label>
                )}
