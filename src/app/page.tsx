...
export default async function LandingPage() {
  ...
  return (
    <div
      className="landing-focus min-h-screen bg-paper text-ink antialiased"
      style={{ fontFamily: APPLE_FONT }}
    >
      {/* Sticky nav */}
      <header className="sticky top-0 z-50 bg-paper/80 backdrop-blur-xl border-b border-smoke">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-[68px]">
            ...
            <nav className="hidden md:flex items-center gap-8" aria-label="Primary">
              {NAV_LINKS.map((l) => (
                <Link
                  key={l.label}
                  href={l.href}
                  className="text-[15px] text-graphite hover:text-ink transition-colors rounded"
                >
                  {l.label}
                </Link>
              ))}
            ...
            <div className="flex items-center gap-2">
              ...
              <Link
                href="/register"
                className="bg-ink text-white px-5 py-2.5 rounded-full text-[15px] font-medium hover:bg-graphite transition active:scale-95 shadow-[0_8px_24px_-8px_rgba(0,0,0,0.4)]"
              >
                {h('nav.signUp')}
              </Link>
            </div>
      ...
      <main>
        ...
        <section className="relative overflow-hidden">
          ...
          <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pt-20 pb-12 md:pt-28 md:pb-16 text-center">
            ...
            <p
              className="ej-hero-anim text-lg md:text-[21px] leading-relaxed text-graphite mb-10 max-w-3xl mx-auto"
              style={heroDelay(220)}
            >
              {hh('subtitle')}
            </p>
            ...
            >
              <Link
                href="/register"
                className="group inline-flex items-center justify-center gap-2 bg-ink text-white px-8 py-3.5 rounded-full text-[17px] font-medium hover:bg-graphite transition active:scale-95 shadow-[0_16px_40px_-12px_rgba(22,22,22,0.5)]"
              >
                {h('hero.ctaStart')}
              ...
              <Link
                href="#features"
                className="inline-flex items-center justify-center gap-2 text-ink px-8 py-3.5 rounded-full text-[17px] font-medium hover:underline underline-offset-4"
              >
                {h('hero.ctaSee')}
              </Link>
            </div>
        ...
        {/* Trades marquee */}
        <section className="border-y border-smoke py-7">
        ...
        <section id="features" className="py-20 md:py-28 scroll-mt-16">
          <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            <Reveal className="text-center max-w-2xl mx-auto mb-12">
              ...
              <p className="text-lg md:text-[19px] text-graphite">{h('features.subtitle')}</p>
            </Reveal>
            <Reveal delay={120}>
              <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-px bg-smoke border border-smoke rounded-[28px] overflow-hidden">
                {FEATURE_GRID.map((f) => (
                  <div
                    key={f.key}
                    className="group bg-white p-6 md:p-7 flex flex-col items-center text-center gap-3 hover:bg-smoke transition-colors"
                  >
                    <f.icon
                      size={26}
                      strokeWidth={1.5}
                      className="text-zinc-800 transition-all duration-300 group-hover:text-ink group-hover:scale-110"
                    />
                    <span className="text-[13px] font-medium text-graphite group-hover:text-ink transition-colors">
                      {h(`features.${f.key}`)}
                    </span>
                  </div>
                ))}
          ...
          <div className="relative max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="grid lg:grid-cols-2 gap-12 lg:gap-20 items-center">
              ...
              <div className="order-1 lg:order-2">
                <Reveal>
                  <p className="text-[13px] font-semibold tracking-[0.18em] text-lime uppercase mb-5">
                    {h('copilot.eyebrow')}
                  </p>
                ...
                <Reveal delay={300}>
                  <Link
                    href="/register"
                    className="group inline-flex items-center gap-2 text-lime font-medium text-[17px] hover:text-white transition-colors"
                  >
                    {h('copilot.cta')}
                    <ArrowRight size={18} className="transition-transform group-hover:translate-x-1" />
                  </Link>
        ...
        <section className="py-20 md:py-28">
          <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div className="grid grid-cols-2 lg:grid-cols-4 gap-10 md:gap-8">
              {STATS.map((s, i) => (
                <Reveal key={s.key} delay={i * 90} className="text-center lg:text-left">
                  <p className="text-[12px] font-semibold tracking-[0.16em] text-ink uppercase mb-2">
                    {h(`stats.${s.key}Label`)}
                  </p>
                  <p className="text-5xl md:text-6xl font-bold tracking-[-0.03em] mb-3">{s.value}</p>
                  <p className="text-[14px] text-graphite leading-relaxed max-w-[220px] mx-auto lg:mx-0">
                    {h(`stats.${s.key}Desc`)}
                  </p>
                </Reveal>
              ))}
            ...
            <Reveal className="text-center mt-14">
              <Link
                href="/register"
                className="group inline-flex items-center justify-center gap-2 bg-ink text-white px-8 py-3.5 rounded-full text-[17px] font-medium hover:bg-graphite transition active:scale-95 shadow-[0_12px_32px_-12px_rgba(0,0,0,0.45)]"
              >
                {h('stats.cta')}
                <ArrowRight size={18} className="transition-transform group-hover:translate-x-1" />
              </Link>
        ...
        <section id="how-it-works" className="py-20 md:py-28 bg-paper border-y border-smoke scroll-mt-16">
          <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            <Reveal className="text-center max-w-2xl mx-auto mb-14">
              ...
              <p className="text-lg text-graphite">{h('steps.subtitle')}</p>
            </Reveal>
            <div className="grid md:grid-cols-3 gap-5">
              {STEPS.map((s, i) => (
                <Reveal
                  key={s.n}
                  delay={i * 110}
                  className="relative bg-white rounded-[24px] border border-smoke p-8 transition-all duration-300 hover:-translate-y-1.5 hover:shadow-[0_24px_56px_-20px_rgba(0,0,0,0.18)] hover:border-zinc-300"
                >
                  <span className="absolute top-6 right-7 text-[64px] leading-none font-bold text-zinc-100 select-none">
                    {s.n}
                  </span>
                  <div className="w-12 h-12 rounded-2xl bg-ink text-white flex items-center justify-center mb-5">
                    <s.icon size={22} strokeWidth={1.6} />
                  </div>
                  <h3 className="text-[19px] font-semibold tracking-tight text-ink mb-2">
                    {h(`steps.${s.key}Title`)}
                  </h3>
                  <p className="text-[15px] text-graphite leading-relaxed">{h(`steps.${s.key}Desc`)}</p>
                </Reveal>
              ))}
            </div>
          </div>
        ...
        <section id="regions" className="py-20 md:py-28 scroll-mt-16">
          <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            <Reveal className="text-center max-w-2xl mx-auto mb-14">
              ...
              <p className="text-lg text-graphite">{h('canada.subtitle')}</p>
            </Reveal>
            <div className="max-w-4xl mx-auto">
              <Reveal delay={120}>
                <div className="bg-white rounded-[24px] border border-smoke p-8 transition-shadow duration-300 hover:shadow-[0_16px_48px_-16px_rgba(0,0,0,0.12)]">
                  <div className="flex items-center gap-3 mb-5">
                    <div className="w-11 h-11 rounded-2xl bg-smoke text-ink flex items-center justify-center">
          ...
          <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
            <Reveal>
              <div className="grid lg:grid-cols-2 gap-10 items-center bg-paper rounded-[32px] border border-smoke p-8 md:p-14">
                <div>
                  <p className="text-[13px] font-semibold tracking-[0.18em] text-ink uppercase mb-5">
                    {h('directory.eyebrow')}
                  </p>
                  <h2 className="text-3xl md:text-[40px] leading-[1.08] font-bold tracking-[-0.02em] mb-4">
                    {h('directory.title')}
                  </h2>
                  <p className="text-[17px] text-graphite mb-8 leading-relaxed">
                    {h('directory.subtitle')}
                  </p>
                  <div className="flex flex-col sm:flex-row gap-3">
                    <Link
                      href="/directory"
                      className="group inline-flex items-center justify-center gap-2 bg-ink text-white px-6 py-3 rounded-full text-[15px] font-medium hover:bg-graphite transition active:scale-95"
                    >
                    ...
                    <Link
                      href="/register"
                      className="inline-flex items-center justify-center gap-2 text-ink px-6 py-3 rounded-full text-[15px] font-medium hover:underline underline-offset-4"
                    >
                      {h('directory.ctaList')}
                    </Link>
                  </div>
                </div>
                <div className="bg-white rounded-[24px] border border-smoke p-6">
                  <p className="text-[12px] font-semibold uppercase tracking-[0.14em] text-zinc-400 mb-4">
                    {h('directory.cardTitle')}
                  </p>
                  <div className="space-y-3 text-[14px]">
                    <div className="flex items-center gap-3 p-3.5 rounded-2xl bg-paper border border-smoke">
                      <Search size={18} className="text-zinc-700 shrink-0" />
                      <p className="text-graphite">
                        {h('directory.card1Pre')}
                        <span className="font-semibold text-ink">{h('directory.card1Bold')}</span>
                      </p>
                    </div>
                    <div className="flex items-center gap-3 p-3.5 rounded-2xl bg-paper border border-smoke">
                      ...
                      <p className="text-graphite">
                        ...
                        <span className="font-semibold text-ink">{h('directory.card2Bold')}</span>
                        {h('directory.card2Post')}
                      </p>
                    </div>
                    <div className="flex items-center gap-3 p-3.5 rounded-2xl bg-smoke border border-smoke">
                      <Bell size={18} className="text-ink shrink-0" />
                      <p className="text-graphite">
                        {h('directory.card3Pre')}
                        <span className="font-semibold text-ink">{h('directory.card3Bold')}</span>
                      </p>
                    </div>
                  </div>
                </div>
        ...
        <section className="relative pb-24 md:pb-32 overflow-hidden">
          ...
          <div className="relative max-w-3xl mx-auto px-4 sm:px-6 text-center">
            <Reveal>
              ...
              <Link
                href="/register"
                className="group inline-flex items-center justify-center gap-2 bg-ink text-white px-9 py-4 rounded-full text-[17px] font-medium hover:bg-graphite transition active:scale-95 shadow-[0_16px_40px_-12px_rgba(22,22,22,0.5)]"
              >
                {h('finalCta.cta')}
                <ArrowRight size={18} className="transition-transform group-hover:translate-x-1" />
              </Link>
      ...
      {/* Footer */}
      <footer className="border-t border-smoke bg-paper">
